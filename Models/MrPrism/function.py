...
#%% These functions are written by Soumick Chatterjee (@soumickmj), for running baseline experiments for PULASki, by adapting train_rec and rec_validate functions.

def prep_tensors(imgs, lbls=None):
    ...
    top = (model_image_size - h) // 2
    bottom = model_image_size - h - top
    left = (model_image_size - w) // 2
    right = model_image_size - w - left
    padding = (top, bottom, left, right)

    imgs = F.pad(imgs, padding)
    if lbls is not None:
        lbls = F.pad(lbls, padding)
        return imgs, lbls, padding
    else:
        return imgs, padding

def unpad(imgs_padded, padding):
    top, bottom, left, right = padding
    return imgs_padded[..., left:-right if right != 0 else None, top:-bottom if bottom != 0 else None]

def train_batch_PULASki(net, imgs, lbls, shuffle_weights=0.3): 
    imgs, lbls, padding = prep_tensors(imgs, lbls)

    b_size,c,w,h = lbls.size()

    cond = torch.ones(b_size, c).to(dtype=imgs.dtype, device=imgs.device) *  (1/c)
    cond = cond.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand(b_size, c, 1, w, h)
    cond = rearrange(cond, "b c a h w -> b (c a) h w")

    '''preprocess the true_masks'''
    true_mask_ave = torch.zeros(b_size, 1, w, h).to(dtype=imgs.dtype, device=imgs.device)
    true_mask_marg = []
    for n in range(lbls.size(1)):
        rater_n_true = lbls[:, n, :, :].unsqueeze(1)
        true_mask_ave += rater_n_true / lbls.size(1)  # Averaging the masks
        true_mask_marg.append(rater_n_true)
    true_mask_p = true_mask_ave

    mask_pred, aux = net(imgs, cond)
    preds, mergfs = aux['maps'], aux['mergfs']
    mpt = torch.sigmoid(unpad(mask_pred, padding))

    pred_stack = torch.stack(preds)  # 7, b, c, w, w
    pred_stack_t = torch.sigmoid(pred_stack)
    ...
    with autocast(False):
        shuffle_loss = nn.BCELoss()(cond_pred.float(), cond_rater.float())
    '''end'''
    ...
    last_rec = ssim_loss(mask_pred[:, 0, :, :].unsqueeze(1), true_mask_p[:, 0, :, :].unsqueeze(1))
    this_rec = ssim_loss(true_mask[:, 0, :, :].unsqueeze(1), mask_pred.detach()[:, 0, :, :].unsqueeze(1))
    rec_loss = last_rec + this_rec
    '''END'''

    loss = rec_loss + shuffle_weights * shuffle_loss
    return loss, mpt

def val_batch_PULASki(net, imgs, lbls, selected_lbl): 
    imgs, lbls, padding = prep_tensors(imgs, lbls)
    
    b_size,c,w,h = lbls.size()
    
    with torch.no_grad():
        cond = torch.ones(b_size,c).to(dtype=imgs.dtype, device = imgs.device) *  (1/lbls.shape[1])
        cond = cond.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand(b_size, c, 1, w, h)
        cond = rearrange(cond,"b c a h w -> b (c a) h w")
        ...
        with autocast(False):
            loss = nn.BCELoss()(mpt.float(), selected_lbl.float())

    return loss, mpt

def infer_batch_PULASki(net, imgs, n_raters): 
    imgs, padding = prep_tensors(imgs)
    
    with torch.no_grad():
        collect_preds = []
        for _ in range(n_raters):
            cond = torch.ones(imgs.shape[0], n_raters).to(dtype=imgs.dtype, device = imgs.device) *  (1/n_raters)
            cond = cond.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand(imgs.shape[0], n_raters, 1, imgs.shape[-2], imgs.shape[-1])
            cond = rearrange(cond,"b c a h w -> b (c a) h w")
            ...
            collect_preds.append(torch.sigmoid(unpad(mask_pred, padding)))

    return collect_preds

#%% Existing functions from the original codebase, not utilised in the PULASki experiments
