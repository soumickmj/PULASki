...
class UnetStageBlock(nn.Module):
    def __init__(self, stage, up_in, x_in, n_out, ratio, n_raters):
        super().__init__()
        # super(UnetBlock, self).__init__()
        up_out = x_out = n_out // 2
        self.x_conv = nn.Conv2d(x_in, x_out, 1)
        self.g_fc = nn.Linear(n_raters,x_out * 2)
        self.tr_conv = nn.ConvTranspose2d(up_in, up_out, 2, stride=2)
        self.stage = stage

        self.bn = nn.BatchNorm2d(n_out)

        self.pointwise = nn.Conv2d(n_raters, n_out, kernel_size=1)
...
class TransUNet(nn.Module):
    ...
    def __init__(self, args, resnet='resnet34', num_classes=2, pretrained=False,
        ...
        self.up1 = UnetStageBlock(self.stages[3], 512, 256, 256,16, n_raters = n_raters)
        self.up2 = UnetStageBlock(self.stages[2], 256, 128, 256,8, n_raters = n_raters)
        self.up3 = UnetStageBlock(self.stages[1], 256, 64, 256,4, n_raters = n_raters)
        self.up4 = UnetStageBlock(self.stages[0], 256, 64, 256,2, n_raters = n_raters)

        self.up5 = nn.ConvTranspose2d(256, self.num_classes, 2, stride=2)
        self.munet = MUNet(args, num_classes = num_classes, n_raters = n_raters)
        
        '''~~~ self definition ~~~'''
        self.fc = nn.Linear(self.n_raters,dim)
    ...
    def forward(self, x, cond, mod = 'train'):
        ...
        maps = [mapsin[:,i,:,:] for i in range(self.n_raters)]

        '''~~~ 0: ENDs ~~~'''

        pred_stack = torch.stack(maps) #n_raters,b,c,w,w
...
class MUNet(nn.Module):
    ...
    def __init__(self, args, resnet='resnet34', num_classes=2, pretrained=False, n_raters=7):
        super().__init__()
        # super(ResUnet, self).__init__()
        self.n_raters = n_raters
        drop_path=0.05
        patch_sizes = (8,8,8,8)
        num_heads = (4,4,4,n_raters)
        last_chs = (num_classes+3,16,16,16)
        num_chs = (16, 16, 16, n_raters)
    ...
    def forward(self, x, heatmap):
        ...
        # x = self.stages[3](x,x)
        x = self.upsample(x)
        x = rearrange(x, "b (g c) h w -> b g c h w", g = self.n_raters)
        outlist = [x[:,i,:,:] for i in range(self.n_raters)]
        # outlist.append(x.item()[:,i,:,:] for i in range(self.n_raters))


        return sum(outlist)/ self.n_raters, x
...
class UNet(nn.Module):
    ...
    def __init__(self, args, resnet='resnet34', in_chans=3, num_classes=2, pretrained=False):
        super().__init__()
        # super(ResUnet, self).__init__()
        ...
        layers = list(base_model(pretrained=pretrained,inplanes = in_chans).children())[:cut]
        self.check_layer = layers
        base_layers = nn.Sequential(*layers)
        self.rn = base_layers
