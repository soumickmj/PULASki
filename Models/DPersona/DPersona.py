...
from .ProbUBase.unet_blocks import *
from .ProbUBase.unet import Unet
from .ProbUBase.utils import init_weights,init_weights_orthogonal_normal, l2_regularisation
...
class Encoder(nn.Module):
    ...
    """
    def __init__(self, input_channels, num_filters, no_convs_per_block, initializers, padding=True, posterior=False, num_experts=4):
        ...
        if posterior:
            ...
            self.input_channels += num_experts
...
class AxisAlignedConvGaussian(nn.Module):
    ...
    """
    def __init__(self, input_channels, num_filters, no_convs_per_block, latent_dim, initializers, posterior=False, num_experts=4):
        ...
        if self.posterior:
            ...
            self.name = 'Prior'
        self.encoder = Encoder(self.input_channels, self.num_filters, self.no_convs_per_block, initializers, posterior=self.posterior, num_experts=num_experts)
...
class DPersona(nn.Module):
    def __init__(self, input_channels=1, num_classes=1, num_filters=[16,32,64,128,256],
        ...
        self.prior = AxisAlignedConvGaussian(self.input_channels, self.num_filters, self.no_convs_per_block, self.latent_dim,  self.initializers, num_experts=self.num_experts).to(device)
        self.posterior = AxisAlignedConvGaussian(self.input_channels, self.num_filters, self.no_convs_per_block, self.latent_dim, self.initializers, posterior=True, num_experts=self.num_experts).to(device)
